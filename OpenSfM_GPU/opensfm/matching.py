...
def match_images(data, ref_images, cand_images):
    ...
    # Match them !
    return match_images_with_pairs(data, exifs, ref_images, cand_images, pairs), preport


def match_images_with_pairs(data, exifs, ref_images, cand_images, pairs):
    """ Perform pair matchings given pairs. """

    # Store per each image in ref for processing
    per_image = {im: [] for im in ref_images}
    for im1, im2 in pairs:
        if im1 in cand_images:
    ...
    logger.info(
        'Matched {} pairs for {} ref_images to {} cand_images {} '
...
def match_unwrap_args(args):
    ...
    logger.info('Image {} matches: {} out of {} in {}s'.format(
        im1, num_matches, len(candidates), wall_time))
    ...
    ctx.data.save_report(io.json_dumps(report), 'matches/{}.json'.format(im1))
