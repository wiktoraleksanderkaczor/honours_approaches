...
def detect(args):
    try:
        ...
        data.delete_features(image)
        data.delete_report('features/{}.json'.format(image))
        logger.error('Deleting features and report for {}'.format(image))