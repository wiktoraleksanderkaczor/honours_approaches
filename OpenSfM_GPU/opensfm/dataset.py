...
class DataSet(object):
    ...
    def cand_images(self):
        """List of file names of all images in the dataset."""
        cand_path = os.path.join(self.data_path, 'image_match.txt')
        if os.path.isfile(cand_path):
            logger.info('Loading {} for image candidates'.format(cand_path))
            cand_list = open(cand_path).read().splitlines()
            for line in cand_list:
                if line not in self.images():
                    cand_list.remove(line)
            return cand_list
        else:
            logger.info("No image pairs specificed, performing exhaustive search")
            return self.image_list
    ...
    def delete_features(self, image):
        try:
            os.remove(self._feature_file(image))
        except:
            return
